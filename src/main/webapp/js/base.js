...
createGame = function() {
	...
	var request = gapi.client.endpoints.createGame({
		'name' : name,
...
print = function(game) {
	...
	}, objStringified = JSON.stringify(obj), objStringifiedAndEncoded = encodeURIComponent(objStringified);
	...
	var gameLink = "https://findteamforgame.appspot.com/showgame.html?"
			+ objStringifiedAndEncoded;
	action.setAttribute("href", gameLink);
	action.setAttribute("class",
			"button button-linkbutton button-linkbutton-shadow");
...
registerForGame = function() {
	var objStringifiedAndEncoded = location.search.substr(1), // substr(1) нужен, чтобы убрать знак вопроса из начала строки
